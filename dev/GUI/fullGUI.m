...
% Last Modified by GUIDE v2.5 17-Feb-2015 11:25:42
...
try
    ...
    % been established.
        ...
        handles.injection.x_offset = 2330.45; %to the right camera view in microns
        handles.injection.y_offset = 35215.8; %away from camera
       
        handles.injection.pauseTime = 120; % in seconds
        
        handles.injection.depth_1 = 250;
        handles.injection.depth_2 = 450;
...
%         end
   ...
   ' image(in_frame);'...
    ...
    h = zoom;
    setAllowAxesZoom(h,handles.cameraAxes,true);
    handles.pntPlot=[];
...
function calibratedPos = calibrateValue(type,dir,pos)
if strcmp(type,'injector')
    if strcmp(dir,'x')
       calibratedPos= 75.4568433838050*pos-37.9291527280028 %add offset
    elseif strcmp(dir,'y')
        calibratedPos= 75.4275913665569*pos+36.6466653647506;
    end
    !!Update drill calibrations!!
elseif strcmp(type,'drill') 
    if strcmp(dir,'x')
       calibratedPos = 75.4568433838050*pos-37.9291527280028; %add offset
    elseif strcmp(dir,'y')
        calibratedPos= 75.4275913665569*pos+36.6466653647506;
...
function setStageVelocity(handles,vel)
...
setStageVelocity(handles,scaledVel);
...
handles.injection.depth_1 = depth;
guidata(hObject,handles);
...
depth = str2double(get(hObject,'String'));
handles.injection.depth_2 = depth;
guidata(hObject,handles);
...
centerPnt = [ 1920/2,1080/2];
pnts = size(handles.data.markedPnts);
setStageVelocity(handles,100);
for i=1:pnts(1,2)
    cur_x = handles.data.markedPnts(1,i)
    pixel_dist_x = pdist([centerPnt(1,1),0;cur_x,0],'euclidean')
    if(centerPnt(1,1)>cur_x)pixel_dist_x= pixel_dist_x*-1;end
    pnt_x = calibrateValue('injector','x',pixel_dist_x)
    
    cur_y =handles.data.markedPnts(2,i)
    pixel_dist_y =  pdist([0,centerPnt(1,2);0,cur_y],'euclidean')
    if(centerPnt(1,2)<cur_y)pixel_dist_y= pixel_dist_y*-1;end
    pnt_y = calibrateValue('injector','y',pixel_dist_y)
    
    % 	add offsets  to drill positions.
    pnt_x = pnt_x - handles.injection.x_offset;
    pnt_y = pnt_y - handles.injection.y_offset;
    
    translate(handles.sutter,[pnt_x pnt_y 0]);
    updatePanel(handles.sutter);
    
     %Step injector in Z 
     stepInjectorZ(handles,pnt_x, pnt_y, z_backStep);   
...
while(~pierced)
    ...
    moveTo(handles.sutter,[x_in y_in curZ]); 
        ...
        if(handles.skull_pierced) % this now represents- when the needle touches dura- passes the silicon elastomer
            ...
            moveTo(handles.sutter,[0 0 handles.injection.depth_1]);
            !%Inject 1
            inject(handles);
            pause(handles.injection.pauseTime);
            
            moveTo(handles.sutter,[0 0 handles.injection.depth_2]);
            !%Inject 2
            inject(handles);
            pause(handles.injection.pauseTime);
...
function acquireDAQProbePulse(handles,acquisition_type, rate, data_update_size,time_to_sample)
           
if (~handles.in_s.IsDone) 
handles.in_s.Rate = rate;
handles.in_s.DurationInSeconds = time_to_sample+.5; % added .5 secs to ensure proper obj cleanup on board
%                 obj.input_s.IsContinuous = true;
handles.in_s.NotifyWhenDataAvailableExceeds = data_update_size;
end
...
lh = addlistener(handles.in_s,'DataAvailable', @(src,event)analyzeTrigger(handles,event,rate,start_time_obj,startTime_seconds,time_to_sample));

handles.DATA_AVAILABLE = lh ;

if strcmp(acquisition_type,'foreground')
     cprintf([0 1 0],'***RECORDING FOREGROUND*** \n');
     [data,time] = handles.in_s.startForeground;
%                     obj.analyzeCurrentSignal(data,rate);
elseif strcmp(acquisition_type,'background')
      cprintf([0 1 0],'***RECORDING BACKGROUND*** \n');
      handles.in_s.startBackground();
...
function stop_drill = analyzeTrigger(handles,event,sampling_rate,start_time_obj, start_time_seconds,time_to_sample)
%           time = event.TimeStamps;
    ...
    if (curTime < (start_time_seconds+time_to_sample) && ~stop_drill)
        stop_drill = analyzeCurrentSignal(handles,data,sampling_rate);
        else
    ...
    if(stop_drill)
        %delete event listener 
        handles.skull_pierced = true;
        cprintf([1 .5 .5], '****PIERECED****\n');
    end 
function stop_drill = analyzeCurrentSignal(handles,data,rate)
            hold on;
%             plot(time,data);
...
%             d_fft_power = (conv(d_fft,conj(d_fft))/Nsamps^2);
            ...
            bot_lim_ind = floor((length(data)*handles.freq_bot/Fs)-1);
            top_lim_ind = ceil((length(data)*handles.freq_top/Fs)+1);
...
%             fft_maxes = [fft_maxes, max_val_fft];
            ...
            handles.lifetime_logged_integral_of_fft = [handles.lifetime_logged_integral_of_fft; integral_of_fft ];
            handles.current_measurement_log_of_fft = [handles.current_measurement_log_of_fft; integral_of_fft];
            
            cprintf([0 1 0], 'Integral of subsample_fft: %d \n', integral_of_fft);
            %Evaluation of pierced skull. Greater than background and .6 of the maximum signal 
            if (integral_of_fft > handles.mouse_fft_background_signal &&  integral_of_fft >= handles.mouse_fft_attenuation_factor*handles.mouse_fft_max_signal)
                stop_drill = true;
            else
                stop_drill = false;
            end
...
if(strcmp(h.Enable,'on'))
    h.ButtonDownFilter = @zoom_Callback;
    h.Enable = 'off';
else
     h.ButtonDownFilter = @zoom_Callback;
    h.Enable = 'on';
